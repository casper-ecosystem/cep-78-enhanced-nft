...
```bash
casper-client put-deploy -n https://node.testnet.casper.network/rpc --chain-name "casper-test" --payment-amount 500000000000 -k keys/secret_key.pem --session-path contract/target/wasm32-unknown-unknown/release/contract.wasm \
...
| 171  | MissingTokenIssuerIdentifierKey             |

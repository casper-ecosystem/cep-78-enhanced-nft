...
export default class Client {
  ...
  private readonly _events: Record<
    string,
    ((event: WithTransactionInfo<CEP78Event>) => void)[]
  > = {};
  ...
  public addEventListener(
    name: string,
    listener: (event: CEP78EventResult) => void
  ) {
  ...
  public removeEventListener(
    name: string,
    listenerToRemove: (event: CEP78EventResult) => void
  ) {
    ...
    const filterListeners = (listener: (event: CEP78EventResult) => void) =>
   ...
   * @param listener - The callback function to execute when the event occurs.
   */
  public on(name: string, listener: (event: CEP78EventResult) => void) {
        ...
        (await this.parseExecutionResult(executionResult))
          ?.map(
            (result) =>
              ...
              }) as unknown as CEP78EventResult
          )
  ...
  /**
   * Parses the execution result to extract CEP78 events.
   *
   ...
   * @returns A `Promise` resolving to an array of parsed CEP78 events or `undefined` if no valid events were found.
   */
  private async parseExecutionResult(
    result: ExecutionResult
  ): Promise<CEP78Event[] | undefined> {
   ...
   */
  private emit(event: CEP78EventResult) {

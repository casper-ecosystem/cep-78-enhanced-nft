...
use crate::{
    constants::{
        ...
        ENTRY_POINT_UPDATED_RECEIPTS, HASH_BY_INDEX, HASH_KEY_NAME, HOLDER_MODE, IDENTIFIER_MODE,
        INDEX_BY_HASH, INSTALLER, JSON_SCHEMA, MAX_TOTAL_TOKEN_SUPPLY, METADATA_CEP78,
        METADATA_CUSTOM_VALIDATED, METADATA_MUTABILITY, METADATA_NFT721, METADATA_RAW,
        MIGRATION_FLAG, MINTING_MODE, NFT_KIND, NFT_METADATA_KIND, NUMBER_OF_MINTED_TOKENS,
        OPERATOR, OWNED_TOKENS, OWNERSHIP_MODE, PAGE_DICTIONARY_PREFIX, PAGE_LIMIT, PAGE_TABLE,
        RECEIPT_NAME, REPORTING_MODE, TOKEN_COUNTS, TOKEN_ISSUERS, TOKEN_OWNERS,
...
pub extern "C" fn mint() {
    ...
    if let OwnerReverseLookupMode::Complete = utils::get_reporting_mode() {
        ...
        let mut page = if !page_table[page_table_entry as usize] {
            ...
            // match storage::dictionary_get::<Vec<bool>>(page_uref,
            // &page_item_key).unwrap_or_revert() {         Some(single_page) =>
            // single_page,         None => {
            ...
            storage::dictionary_get::<Vec<bool>>(page_uref, &page_item_key)
                .unwrap_or_revert()
...
pub extern "C" fn transfer() {
    ...
    if let OwnerReverseLookupMode::Complete = utils::get_reporting_mode() {
        ...
        let mut source_page =
            storage::dictionary_get::<Vec<bool>>(page_uref, &source_page_item_key)
                .unwrap_or_revert()
